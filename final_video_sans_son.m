...
video = VideoReader('Mseq3.avi','Tag','My video');

F = video.NumberOfFrames - 2; %drop last frame
%F=200;
H = video.Height;
W = video.Width;
fps=video.FrameRate;

frames = read(video, [1 F]);
frames = squeeze(mean(frames,3)); % grayscale
...
figure
[~,rawx,rawy,rawframe] = textread('TrueSeq3.xls', '%d %f %f %d', 'headerlines', 1);
truex=interp1(rawframe,rawx,1:F,'linear','extrap');
plot(1:F,truex,'-');hold all
[m,l_f]=l_f=max(q(:,1,:)+q(:,2,:),[],1);
plot(l_f)
rectangle('Position',[0, 76, F, 111-76],'LineWidth',2,'FaceColor','r');

set(p,'FaceAlpha',0.5);

figure()
imagesc(Qs)
imshow(frames(:,:,30)/255)
